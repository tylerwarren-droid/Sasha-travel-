...
export default function PhuQuocPage() {
  ...
  const [photoQuery, setPhotoQuery] = useState('Phu Quoc beach luxury resort Vietnam')
  ...
  const handleSashaResponse = useCallback((text: string) => {
    ...
    const golfSpots = ['vinpearl golf', 'phu quoc golf', 'golf phu quoc']
    const pqSpots = ['long beach', 'sao beach', 'bai dai', 'grand world', 'sun world', 'vinpearl', 'duong dong', 'phu quoc', 'pearl island', 'apec', 'sun group']
    const wellness = ['spa', 'massage', 'wellness', 'resort pool', 'beach club']
    for (const spot of golfSpots) {
      if (lower.includes(spot)) { setPhotoQuery('Vinpearl golf Phu Quoc Vietnam'); return }
    }
    for (const spot of pqSpots) {
      if (lower.includes(spot)) { setPhotoQuery(`${spot} Phu Quoc Vietnam luxury`); return }
    }
    for (const spot of wellness) {
      if (lower.includes(spot)) { setPhotoQuery('luxury spa beach resort Phu Quoc Vietnam'); return }
    }
  }, [speakFn])
  ...
  return (
    <main className="h-screen flex flex-col overflow-hidden" style={{ background: '#080810' }}>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/5 flex-shrink-0" style={{ background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)' }}>
        ...
        <div className="text-xs px-2 py-1 rounded-full border" style={{ color: '#00B4D8', borderColor: 'rgba(0,180,216,0.3)', background: 'rgba(0,180,216,0.1)' }}>
          MoT Partner
        </div>
      </div>
