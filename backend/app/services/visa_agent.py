...
Always provide: visa type needed, cost, processing time, application link, passport validity required, and any entry restrictions.
Collect: traveler's nationality/passport, destination country, and approximate travel dates."""
        ...
        if response.stop_reason == "tool_use":
            ...
            for block in response.content:
                if block.type == "tool_use":
                    inp = block.input
                    if block.name == "check_visa_requirements": result = await check_visa_requirements(**inp)
