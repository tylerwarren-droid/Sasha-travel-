...
async def send_pet_booking_email(provider_email, provider_name, guest_name, guest_email, service_requested, preferred_date, preferred_time, guest_phone="", dog_name="", dog_breed="", dog_age="", special_notes="", duration="", hotel_name="", hotel_address="") -> dict:
    ...
    pickup = (hotel_name + ", " + hotel_address) if hotel_address else "To be confirmed"
    dname = dog_name or "TBC"
    dbreed = dog_breed or "TBC"
    dage = dog_age or "TBC"
    ...
    body = "\n".join([
        ...
        "Pickup: " + pickup,
        "",
        "Dog: " + dname + " / Breed: " + dbreed + " / Age: " + dage,
        "Notes: " + notes,
        "",
        ...
        async with httpx.AsyncClient() as http:
            await http.post("https://api.resend.com/emails",
                ...
                json={"from": SASHA_FROM_EMAIL, "to": [provider_email], "reply_to": guest_email,
                    "subject": "Pet Service Booking - " + guest_name + " - " + preferred_date, "text": body})
            await http.post("https://api.resend.com/emails",
                headers={"Authorization": "Bearer " + RESEND_API_KEY, "Content-Type": "application/json"},
                json={"from": SASHA_FROM_EMAIL, "to": [guest_email, SASHA_NOTIFY_EMAIL],
                    "subject": "Pet care booked - " + provider_name, "text": guest_body})
        return {"sent": True}
    except Exception as e:
...
async def call_pet_provider(provider_phone, provider_name, guest_name, service_requested, preferred_date, preferred_time, dog_name="", dog_breed="", duration="", hotel_name="", hotel_address="") -> dict:
    ...
    pickup = ("from " + hotel_name + " at " + hotel_address) if hotel_address else "from their accommodation"
    breed = dog_breed or "dog"
    dname = dog_name or "their dog"
    dur = duration or "about an hour"
    task = "You are a travel concierge booking a " + service_requested + " for guest " + guest_name + "'s " + breed + " named " + dname + " on " + preferred_date + " at " + preferred_time + " for " + dur + ". Pickup " + pickup + ". Confirm availability and get a reference. Be warm and professional."
    try:
        async with httpx.AsyncClient() as http:
...
def find_expat_pet_groups(location) -> dict:
    return {
        ...
        "facebook": "Search Facebook: '" + location + " Expats' or '" + location + " Dog Owners'"
    }
...
Collect: guest name, dog name/breed/age, location/hotel, service, date/time, duration, guest email. Always ask about special needs."""
